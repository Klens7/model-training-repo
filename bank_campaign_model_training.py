# this is the same script from lesson 3, except added 1 function called 'write metrics to bigquery'
# in training video, we ran this in vertex ai, custom ipynb notebook, ran first cell to pip install then ran the rest
# it will store the performance metrics of the model every time it is run and stores it in a bigquery table we already created
# once your run the cell, it should say metrics inserted successfully. and means model artifact will be saved in GCP service
...
# GCS bucket name might need to be edited so lets check
bucket = storage_client.bucket("sid-ml-ops")

# make sure the csv is already in GCS
def load_data(path):
...
def save_model_artifact(model_name, pipeline):
    ...
    # the artifact will be saved inside "bank_campaign_artifact" in GCS bucket inside sid-ml-ops bucket
    # file should be called xgboost_model.joblib     " its the joblib artifact file"
...
def write_metrics_to_bigquery(algo_name, training_time, model_metrics):
    ...
    # the table called bank_campaign_model_metrics will have the 3 columns from the arguements above (algo_name,etc,etc)
...
def main():
    ## this file name below needs to be in this GCS bucket storage path already before running
    ...
    # model artifact is written to GCS bucket
